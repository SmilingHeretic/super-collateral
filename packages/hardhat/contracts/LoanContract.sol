...
contract LoanContract is KeeperCompatibleInterface {
    ...
    uint256 public overpaymentReturnDeposit;
    uint256 public loanInitiatedTimestamp;
    // status
    // 0 - contract empty waiting for the lender to provide funds and terms
    // 1 - funds and terms provided. Waiting for the borrower to approve the collateral and initiate the loan.
    ...
    function prepareLoan(
        ...
        overpaymentReturnDeposit = msg.value - _loanAmount;
    ...
    function terminateEarly() external onlyLender() {
        ...
        require(status == 1, "Loan already initiated or loan contract empty");
    ...
    function initiateLoan() external onlyBorrower() {
        // Borrower initiates the loan - accepts the terms, receives the funds and temporarily transfers ownership of the collateral.
        require(status == 1, "Loan already initiated or funds and terms not yet provided");
        require(collateralSalaryNFT.getApproved(1) == address(this), "Need to approve collateral salary NFT for this contract before initiating the loan");
        ...
        loanInitiatedTimestamp = block.timestamp;
        status = 2;
    }
    ...
    function onLoanReapid() public {
        ...
        // During this delay period the repayment stream to the lender is still open. We need to return the overpayment to the borrower.
        uint256 amountOverpaid = getAmountRepaid() - (loanAmount + totalInterest);
        uint256 amountToReturn = amountOverpaid;
        if (amountOverpaid > overpaymentReturnDeposit) {
            amountToReturn = overpaymentReturnDeposit;
        }
        (bool sent,) = borrower.call{value: amountToReturn}("");
        require(sent, "Failed to send Ether");
        
        // Send the remainder of deposit for the case of overpayment to the lender.
    ...
    function getAmountRepaid() public view returns (uint256) {
        ...
        return (block.timestamp - loanInitiatedTimestamp) * uint256(repayRate); 
    }
