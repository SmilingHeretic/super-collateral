...
describe("SuperCollateral Dapp", function () {
    ...
    beforeEach(async () => {
        ...
        // await addr0.sendTransaction({to: emmyAddress, value: ethers.utils.parseEther("100")})
        ...
        logBalances = async (name, signer) => {
            ...
            // console.log(await daiContract.symbol(),  await ethers.utils.formatUnits(await daiContract.balanceOf(signer.getAddress())))
        ...
        // check balances: ETH, DAI, LINK
        // await logBalances("Emmy", emmySigner)
        // await logBalances("Employer", employerSigner)
        ...
        // console.log("before", (await cfaContract.getNetFlow(ethxAddress, emmyAddress)).toString())
        ...
        // await logFlows()
        // console.log("after", (await cfaContract.getNetFlow(ethxAddress, emmyAddress)).toString())
        // console.log(await cfaContract.realtimeBalanceOf(ethxAddress, employerAddress, "1000000000"))
        // console.log(await cfaContract.realtimeBalanceOf(ethxAddress, employerAddress, "10000000000"))


        // await salaryAnchor.connect(emmySigner).safeTransferFrom(emmyAddress, dappAddress, 1)
        // await logFlows()
        // await logBalances("Emmy", emmySigner)
    })
    ...
    xit("Emmy opens additional stream to the Dapp.",  async function () {
        await salaryAnchor.connect(emmySigner).updateOutflow(dappAddress, 0.2 * tokenPerMonth)

        expect(await salaryAnchor.ownerOf(1)).to.equal(emmyAddress);
        expect(await (await cfaContract.getNetFlow(ethxAddress, emmyAddress))).to.equal(0.8 * tokenPerMonth)
        expect(await (await cfaContract.getNetFlow(ethxAddress, dappAddress))).to.equal(0.2 * tokenPerMonth)
        expect(await (await cfaContract.getNetFlow(ethxAddress, salaryAnchor.address))).to.equal(0)
